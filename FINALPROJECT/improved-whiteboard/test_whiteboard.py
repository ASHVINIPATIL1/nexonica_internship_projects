...
Tests all features implemented so far with on-screen text display
"""
...
def main():
    ...
    print("  'x' = Clear recognized text from screen")
    print("  'u' = Undo")
    print("  'r' = Redo")
    ...
    show_text = False  # Flag to show/hide text on screen
    ...
    while True:
        ...
        # Display recognized text on screen (NEW!)
        if show_text and recognized_text:
            # Create semi-transparent background for text
            text_y_position = h - 100
            
            # Background box
            cv2.rectangle(result, (20, text_y_position - 40), (w - 20, text_y_position + 30), (0, 0, 0), -1)
            cv2.rectangle(result, (20, text_y_position - 40), (w - 20, text_y_position + 30), (0, 255, 0), 3)
            
            # Display recognized text
            cv2.putText(result, f"Recognized: {recognized_text}", 
                        (30, text_y_position - 5), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
            
            # Instructions to clear
            cv2.putText(result, "Press 'x' to clear text", 
                        (30, text_y_position + 20), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # Instructions
        cv2.putText(result, "Press 's' for shape | 'a' for arrow | 't' for text | 'u' undo | 'r' redo | 'c' clear | 'q' quit", 
                   (palette_x, h - 20), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        ...
        if key == ord('q'):
            ...
            # Text recognition with ON-SCREEN DISPLAY
            ...
            try:
                ...
                # Run OCR
                result_ocr = text_recognizer.recognize_with_confidence(canvas_only)
                
                if result_ocr['text']:
                    recognized_text = result_ocr['text']
                    show_text = True  # Enable on-screen display
                    print(f"✨ Recognized: '{recognized_text}'")
                    print(f"📊 Confidence: {result_ocr['confidence']:.1f}%")
                    print(f"💡 Text is now displayed on screen! Press 'x' to clear.")
                else:
            ...
            show_text = False
            recognized_text = ""
            print("🗑️ Text display cleared from screen")
